...
* Ver 3.52
* Ver 3.52c Ver CP/M-68K : Fixed a bug that an error occurs in array declaration.skyriver
* Ver 3.52b ported to CP/M-68K. Save/load function has not been ported.2022/02/07 skyriver
...
**ram_addr	EQU	$20000		* RAM start address
**ram_size	EQU	$20000		* RAM size


*	INCLUDE	"Basic68k3.41.inc"		* RAM offset definitions
...
Wrmjpv:
	ds.l	1			* BASIC warm start jump vector

Usrjmp:
	ds.w	1			* USR function JMP address
Usrjpv:
	ds.l	1			* USR function JMP vector
...
V_INPT:
	ds.w	1			* non halting scan input device entry point
V_INPTv:
	ds.l	1			* non halting scan input device jump vector

V_OUTP:
	ds.w	1			* send byte to output device entry point
V_OUTPv:
	ds.l	1			* send byte to output device jump vector

V_LOAD:
	ds.w	1			* load BASIC program entry point
V_LOADv:
	ds.l	1			* load BASIC program jump vector

V_SAVE:
	ds.w	1			* save BASIC program entry point
V_SAVEv:
	ds.l	1			* save BASIC program jump vector

V_CTLC:
	ds.w	1			* save CTRL-C check entry point
V_CTLCv:
	ds.l	1			* save CTRL-C check jump vector


Itemp:
		ds.l	1			* temporary integer	(for GOTO etc)

Smeml:
		ds.l	1			* start of memory		(start of program)
...
Sfncl:
		ds.l	1			* start of functions	(end of Program)
...
Svarl:
		ds.l	1			* start of variables	(end of functions)
...
Sstrl:
		ds.l	1			* start of strings	(end of variables)
...
Sarryl:
		ds.l	1			* start of arrays		(end of strings)
...
Earryl:
		ds.l	1			* end of arrays		(start of free mem)
Sstorl:
		ds.l	1			* string storage		(moving down)
Ememl:
		ds.l	1			* end of memory		(upper bound of RAM)
Sutill:
		ds.l	1			* string utility ptr
Clinel:
		ds.l	1			* current line		(Basic line number)
Blinel:
		ds.l	1			* break line		(Basic line number)

Cpntrl:
		ds.l	1			* continue pointer
Dlinel:
		ds.l	1			* current DATA line
Dptrl:
		ds.l	1			* DATA pointer
Rdptrl:
		ds.l	1			* read pointer
Varname:
		ds.l	1			* current var name
Cvaral:
		ds.l	1			* current var address
Lvarpl:
		ds.l	1			* variable pointer for LET and FOR/NEXT

des_sk_e:
		ds.l	6			* descriptor stack end address
...
Asptl:
		ds.l	1			* array size/pointer
Astrtl:
		ds.l	1			* array start pointer
...
Binss:
		ds.l	4			* number to bin string start (32 chrs)

Decss:
		ds.l	1			* number to decimal string start (16 chrs)
		ds.w	1			*
Usdss:
		ds.w	1			* unsigned decimal string start (10 chrs)

Hexss:
		ds.l	2			* number to hex string start (8 chrs)

BHsend:
		ds.w	1			* bin/decimal/hex string end


prstk:
		ds.b	1			* stacked function index

tpower:
		ds.b	1			* remember CORDIC power

Asrch:
		ds.b	1			* scan-between-quotes flag, alt search character

Dimcnt:
		ds.b	1			* # of dimensions

Breakf:
		ds.b	1			* break flag, $00=END else=break
Oquote:
		ds.b	1			* open quote flag (Flag: DATA; LIST; memory)
Gclctd:
		ds.b	1			* garbage collected flag
Sufnxf:
		ds.b	1			* subscript/FNX flag, 1xxx xxx = FN(0xxx xxx)
Imode:
		ds.b	1			* input mode flag, $00=INPUT, $98=READ

Cflag:
		ds.b	1			* comparison evaluation flag

TabSiz:
		ds.b	1			* TAB step size

comp_f:
		ds.b	1			* compare function flag, bits 0,1 and 2 used
*						* : bit 2 set if >
*						* : bit 1 set if =
*						* : bit 0 set if <

Nullct:
		ds.b	1			* nulls output after each line
TPos:
		ds.b	1			* BASIC terminal position byte
TWidth:
		ds.b	1			* BASIC terminal width byte
Iclim:
		ds.b	1			* input column limit
ccflag:
		ds.b	1			* CTRL-C check flag
ccbyte:
		ds.b	1			* CTRL-C last received byte
ccnull:
		ds.b	1			* CTRL-C last received byte 'life' timer
...
VEC_OUT:
	...
	MOVE.w	d0,d1			* copy character
	MOVE.w	#2,d0			* character out
	TRAP	#BDOS			* call BDOS
...
VEC_IN:
	...
	MOVE.w	#6,d0			* direct console I/O
	MOVE.w	#$FE,D1			* get status
	TRAP	#BDOS			* call BDOS

	TST.w	d0			* copy the returned status
...
RETCHR:
	MOVE.w	#6,d0			* direct console I/O
	MOVE.w	#$FF,D1		* get console input data
	TRAP	#BDOS			* call BDOS
...
** change to SYSTEM command
	...
	BNE		getFile			* if not "," get the filename from the line
...
**	MOVEA.l	#ram_addr,a0		* tell BASIC where RAM starts
	MOVEA.l	#ram_strt,a0		* tell BASIC where RAM starts change by skyriver

**	MOVE.l	#ram_size,d0		* tell BASIC how big RAM is
	MOVE.l	sp,d0			* change by skyriver
	SUB.l	a0,d0			* change by skyriver
...
**	BEQ.s		LAB_1359			* loop if null byte : comment out by skyriver
...
LAB_INPUT:
	...
	MOVEQ		#$3b,d0			* set the search character to ";"
	BSR		LAB_SCCA			* scan for CHR$(d0), else do syntax error/warm
*							* start
...
*							* else get string
	...
	MOVEQ		#$3a,d2			* set new search character ':'
	MOVEQ		#$2c,d0			* other search character is ","
...
*	CMP.b		#TK_POINT,d0		* compare with token for POINT ___
...
LAB_2967:
	...
	MOVEQ		#$30-1,d4			* set character to "0"-1
LAB_2968:
...
ofchr	EQU	$23					* '#' the overflow character
...
* process the number string, length in d6, decimal point index in d2
	...
	MOVEQ		#$2e,d4			* set the decimal point character '.'
LAB_U005:
...
LAB_U00C:
	MOVE.w		d2,d6				* copy the write index as the number add 'w' skyriver
*							* string length
...
LAB_U01C:
	MOVEQ		#$30,d0			* default to "0" character
...
LAB_U02A:
	...
	MOVEQ		#$30,d7			* default to "0" character
	CMP.b		#'%',d0			* compare it with "%"
	BEQ.s		LAB_U02B			* if "%" go copy a number character

	MOVEQ		#$20,d7			* default to " " character
	CMP.b		#'#',d0			* compare it with "#"
...
* now add in new digit
	...
	BCS.s		RTS_024			* return if overflow, it should never ever do this
...
LAB_CN	dc.b	"Cann't continue",$00
...
LAB_SMSG:
	...
	dc.b	'Enhanced 68k BASIC Version 3.52c(CP/M-68K)',$0D,$0A,$00
