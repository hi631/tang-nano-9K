...
module NES(input clk, input reset, input ce,
  ...
  //
  wire [7:0]  iodt, rxdt, sardt;
  assign sarcs = addr[15: 4]==12'h402; // 4B xxx0(in.sts+in),xxx1(in.clr),xxx2(out.sts+out) 
  assign sardt = addr[1:0]==2'b00 ? {rxdten,rxdt[6:0]} : 
				 addr[1:0]==2'b10 ? {txbusy,7'h00}     :
				 8'h00;
  wire [7:0] txdt = cpu_dout;
  wire [7:0] rxdt;
  wire txbusy,rxen,rxbusy; 
  wire txen = sarcs && (addr[1:0]==2'b10) && mw_int;
  always @(posedge clk) begin 
	if(rxen) rxdten <= 1;
    else if(sarcs && addr[1:0]==2'b01) rxdten <= 0;
  end
  rs232c_tx_rx uart(
	.RESETB(~CPU_RESET), .CLK(clk),
	.TXD(txs),	.RXD(rxs),
	.TX_DATA(txdt),	.TX_DATA_EN(txen),	.TX_BUSY(txbusy),
	.RX_DATA(rxdt),	.RX_DATA_EN(rxen),	.RX_BUSY(rxbusy));
  reg rxdten;
